	...
	function visibleRoutes() {
		const filtered = routes
			...
			.filter(({ route }) => {
				const matchesQuery =
					!query.trim() ||
					route.name.toLowerCase().includes(query.trim().toLowerCase());
				const matchesGrade =
					gradeFilter === "all" ||
					(gradeFilter === "V6+"
						? gradeValue(route.grade) >= 6
						: route.grade.toUpperCase() === gradeFilter);
				return matchesQuery && matchesGrade;
			});
		if (order === "name")
			filtered.sort((a, b) => a.route.name.localeCompare(b.route.name));
		if (order === "grade")
			filtered.sort(
				(a, b) => gradeValue(a.route.grade) - gradeValue(b.route.grade),
			);
		if (order === "holds")
			filtered.sort((a, b) => b.route.holds.length - a.route.holds.length);
		return filtered;
	}
</script>
...
<section class="route-list" aria-label="Bouldering routes">
	<div class="list-tools">
		...
		<div class="select-row">
			<select aria-label="Filter by grade" bind:value={gradeFilter}>
				{#each grades as grade}<option value={grade}
						>{grade === "all" ? "All grades" : grade}</option
					>{/each}
			</select>
	...
	<div class="routes">
		{#each visibleRoutes() as item, i (item.index)}
			<div
				class:active={item.index === selected}
				class="route-row"
				onfocusin={() => onSelect(item.index)}
				onclickcapture={() => onSelect(item.index)}
			>
				<span class="route-count">{i + 1}</span>
				<input
					class="route-name"
				...
				<button
					...
					aria-label={`Delete ${item.route.name || "unnamed climb"}`}>×</button
				>
			</div>
		{:else}
...
<style>
	...
	.route-row {
		display: grid;
		grid-template-columns: 26px minmax(0, 1fr) 52px 22px;
		gap: 5px;
		align-items: center;
		padding: 7px 20px;
